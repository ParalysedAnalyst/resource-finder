...
class ResourceFinderApp:
    ...
    def __init__(self, root: tk.Tk, fieldteams: pd.DataFrame):
        ...
        self.routes_df = pd.DataFrame()  # results with geometry after OSRM
    ...
    def _build(self):
        ...
        self.bu_combo.set("(Any)"); self.bu_combo.grid(row=2, column=1, sticky="w", padx=(0,8))
        ...
        ttk.Checkbutton(top, variable=self.sla_var, text="Include teams not meeting SLAs (coming soon)").grid(row=2, column=3, sticky="w")

        # minutes bar across both columns just above table/map area
    ...
    def _draw_map(self, show_route: LineString | None = None):
        ...
        # 1) Isochrone polygon (selected minutes)
        if self.iso_gdf is not None:
            minutes = int(self.minutes_var.get())
            sel = self.iso_gdf[self.iso_gdf["minutes"] == minutes]
            if not sel.empty:
                sel.plot(ax=self.ax, facecolor="#b2d235", edgecolor="#294238", alpha=0.2)

        # 2) Teams (filtered if present)
        if not self.filtered.empty:
            gdf = gpd.GeoDataFrame(self.filtered, geometry=[Point(lon, lat) for lon,lat in zip(self.filtered["Longitude"], self.filtered["Latitude"])], crs="EPSG:4326")
            gdf.plot(ax=self.ax, marker="o", markersize=20, color="#50b748", alpha=0.9)

        # 3) Site point
        if self.site_lon is not None and self.site_lat is not None:
            gpd.GeoSeries([Point(self.site_lon, self.site_lat)], crs="EPSG:4326")\
               .plot(ax=self.ax, marker="*", color="#f57821", markersize=120, zorder=5)

        # 4) Selected route
        if show_route is not None:
            gpd.GeoSeries([show_route], crs="EPSG:4326")\
               .plot(ax=self.ax, linewidth=3, color="#294238", zorder=4)
        ...
        if self.iso_gdf is not None:
            minutes = int(self.minutes_var.get())
            sel = self.iso_gdf[self.iso_gdf["minutes"] == minutes]
            if not sel.empty: all_geoms.append(sel.unary_union)
        if not self.filtered.empty:
            pts = [Point(lon, lat) for lon,lat in zip(self.filtered["Longitude"], self.filtered["Latitude"])]
            all_geoms.extend(pts)
        if self.site_lon is not None:
            all_geoms.append(Point(self.site_lon, self.site_lat))

        if all_geoms:
            bounds = gpd.GeoSeries(all_geoms, crs="EPSG:4326").total_bounds  # xmin, ymin, xmax, ymax
            xmin, ymin, xmax, ymax = bounds
            dx, dy = (xmax-xmin)*0.1 or 0.01, (ymax-ymin)*0.1 or 0.01
    ...
    def on_postcode_changed(self, event=None):
        ...
        try:
            ...
            self.bu_combo["values"] = list_business_units(within60) or ["(Any)"]
    ...
    def on_row_select(self, event=None):
        if self.routes_df.empty:
            return  # only draw route after routes have been computed
        ...
        if geom and isinstance(geom, dict) and geom.get("type") in ("LineString","MultiLineString"):
            if geom["type"] == "LineString":
                ...
                # fallback: merge first part of MultiLineString
                line = LineString(geom["coordinates"][0])
