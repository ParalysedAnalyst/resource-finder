...
- route_rank_teams: OSRM for selected teams, returns sorted DataFrame (+ optional geometry)
"""

import math
import pandas as pd
from .api_config import osrm_route  # relative import into package
...
def preselect_by_air_distance(teams_df: pd.DataFrame, site_lon: float, site_lat: float, top_n: int = 20) -> pd.DataFrame:
    if teams_df.empty:
...
def route_rank_teams(
    ...
    """
    Preselect by air distance, call OSRM, return DataFrame sorted by drive time (then km).
    If include_geometry=True, a 'geometry' column is included (GeoJSON LineString/MultiLineString).
    """
